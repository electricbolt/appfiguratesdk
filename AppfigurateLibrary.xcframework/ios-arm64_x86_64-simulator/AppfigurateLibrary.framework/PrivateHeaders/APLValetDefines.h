...
/*
 ...
 Example:
 APLCheckCondition(isProperlyConfigured, nil, @"Foo was not properly configured.");
 
 */
#define APLCheckCondition(condition, result, desc, ...) \
    do { \
        ...
        if (!conditionResult) { \
            @try { \
                ...
                APLDEBUG(@"Valet API Misuse: %s %@", __PRETTY_FUNCTION__, exception.reason); \
                return result;\
            } \
        } \
    } while(0)
